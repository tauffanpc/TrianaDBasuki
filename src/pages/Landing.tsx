...
import { Heart, Send, Sparkles, MessageCircle, Flower2 } from 'lucide-react';
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        const [msg, greet, bg] = await Promise.all([
    ...
    <Layout dayCounter={dayCount} dateStr={dateStr} customBg={background || undefined}>
      <div className="space-y-12 py-4">
        ...
        <motion.div 
          ...
          className="glass-card rounded-[2rem] p-8 space-y-5 optimize-gpu"
        >
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-pink-50 rounded-xl flex items-center justify-center text-pink-500">
              <MessageCircle className="w-5 h-5" />
            </div>
            <h4 className="font-bold text-xs uppercase tracking-[0.15em] text-pink-600 italic">Ceritakan Padaku, Sayang</h4>
          </div>
          <p className="text-xs text-gray-900 leading-relaxed italic">
            "Sayang, kamu bisa sampaikan apapun yang kamu rasakan saat ini padaku. Apapun itu, aku akan selalu mendengarkan dan ada untukmu..."
          </p>
          <div className="relative">
            <textarea
              value={userCurhat}
              onChange={(e) => setUserCurhat(e.target.value)}
              placeholder="Ceritakan isi hatimu padaku, Sayang..."
              className="w-full bg-white/80 border border-white rounded-2xl p-5 text-sm text-gray-900 focus:ring-4 focus:ring-pink-50 focus:border-pink-100 outline-none transition-all min-h-[120px] resize-none shadow-inner italic placeholder:text-gray-400"
            />
            <button
              onClick={handleSendCurhat}
              disabled={isSending || !userCurhat.trim()}
              className="absolute bottom-3 right-3 p-3.5 bg-gradient-to-r from-pink-500 to-rose-400 text-white rounded-xl shadow-lg shadow-pink-100 disabled:opacity-50 disabled:shadow-none transition-all hover:scale-105 active:scale-95 optimize-gpu"
            >
              {isSending ? (
